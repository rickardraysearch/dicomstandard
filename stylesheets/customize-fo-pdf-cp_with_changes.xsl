...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:import href="customizedtitlepages-fo-pdf-cp.xsl"/>	<!-- http://www.sagehill.net/docbookxsl/TitlePageGraphics.html -->
  ...
  <fo:block>
    ...
    <xsl:choose>
      <xsl:when test="$position='center'">
		...
		<xsl:text>- </xsl:text><xsl:value-of select="/d:book/d:chapter/d:informaltable/d:tr[1]/d:td[2]"/><xsl:text> -</xsl:text>
      </xsl:when>
    </xsl:choose>
  </fo:block>
</xsl:template>
