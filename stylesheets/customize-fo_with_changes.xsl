...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:import href="../docbook-xsl-ns-1.78.1/deltaxml-fo-ns/changebars.xsl"/>
	
	<xsl:import href="customizedtitlepages-fo.xsl"/>	<!-- http://www.sagehill.net/docbookxsl/TitlePageGraphics.html -->
	...
	<xsl:attribute-set name="table.table.properties">
		...
		<!-- <xsl:attribute name="table-layout">auto</xsl:attribute> --> <!-- does cause this to be included on <fo:table/>, but makes no difference to the result -->
	</xsl:attribute-set>
...
<!-- we use titles on lists (esp. variablelists) for Defined Terms and Enumerated Values, and do not want bold or too much spacing -->
<!-- so override invocation in fo/lists.xml to use other than the default formal.title.properties (used in fo/formal.xsl) -->

<xsl:template match="d:title" mode="list.title.mode">
  <!--<xsl:call-template name="formal.object.heading">-->
  <xsl:call-template name="ourlist.object.heading">
    <xsl:with-param name="object" select=".."/>
  </xsl:call-template>
</xsl:template>

<!--<xsl:attribute-set name="formal.title.properties" use-attribute-sets="normal.para.spacing">-->
<xsl:attribute-set name="ourlist.title.properties" use-attribute-sets="normal.para.spacing">
	<!-- Do not mess with inherited weight or size or spacing -->
</xsl:attribute-set>

<!--<xsl:template name="formal.object.heading">-->
<xsl:template name="ourlist.object.heading">
  <xsl:param name="object" select="."/>
  <xsl:param name="placement" select="'before'"/>

  <fo:block xsl:use-attribute-sets="ourlist.title.properties">
    <xsl:choose>
      <xsl:when test="$placement = 'before'">
        <xsl:attribute
               name="keep-with-next.within-column">always</xsl:attribute>
...
<!-- The default list.block.spacing is excessive -->
...
<!-- DAC. This template from fo/htmltbl.xsl contains a defect related to column separators when row spanning, so patch it -->
