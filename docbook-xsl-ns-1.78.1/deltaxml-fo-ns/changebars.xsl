...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="use.changebars">1</xsl:param>
  ...
  <xsl:attribute-set name="ancestor.revision.atts">
    <xsl:attribute name="color">
      <xsl:choose>
        ...
        <xsl:when test="@revisionflag='changed' and ((count(text())=1 and count(node())=1) or count(node())=0)"><xsl:value-of select="$nonwrappable.color"/></xsl:when>
        <xsl:otherwise>inherit</xsl:otherwise>
      </xsl:choose>
    </xsl:attribute>
    <xsl:attribute name="background-color">
